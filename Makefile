...
DOCKER_PROJ = super-simple-kiosk
...
help:
	...
	@echo "  test-all          to run all tests with coverage"
	@echo "  test-mqtt         to run MQTT-related tests"
	@echo "  test-browser      to run browser-related tests"
...
add-to-github:
	@gh repo create super-simple-kiosk --public --source=.
	@git branch -M main
	@git remote add origin https://github.com/UpDryTwist/super-simple-kiosk.git
...
test-unit:
	@poetry run pytest tests/unit/ -v --cov=super_simple_kiosk --cov-report=term-missing

test-integration:
	@poetry run pytest tests/integration/ -v --cov=super_simple_kiosk --cov-report=term-missing

test-all:
	@poetry run pytest tests/ -v --cov=super_simple_kiosk --cov-report=term-missing --cov-report=html:htmlcov

test-mqtt:
	@poetry run pytest -m mqtt -v

test-browser:
	@poetry run pytest -m browser -v
